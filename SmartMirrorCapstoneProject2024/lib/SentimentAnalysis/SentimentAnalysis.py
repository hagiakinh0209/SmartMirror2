...
class FER:
    ...
    def detect_emotions(
        self, img: np.ndarray, face_rectangles: NumpyRects
    ) -> list:
        """
        ...
        if face_rectangles is None:
            log.debug("Error, face_rectangles cant be None")
    ...
    def top_emotion(
        ...
        """
        emotions = self.detect_emotions(img=img, face_rectangles = face_rectangles)
        top_emotions = [
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        try:
            ...
            fer.analyzeSentiment(singleton.getImage())
            cv2.waitKey(33) 
        except:
            import traceback
            traceback.print_exc()
